...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "main", primary: true do |main|
	...
	# inside the guest, it should offer a balancer on port 80
    ...
    main.vm.provider :virtualbox do |vb|
      ...
      #vb.gui = false
	  # set the display settings: VRAM, VMSVGA and 3D acceleration
	  # to apply when VBox > 6.1.14
	  vb.customize ["modifyvm", :id, "--vram", "16"]
	  vb.customize ["modifyvm", :id, "--graphicscontroller", "vmsvga"]
	  vb.customize ["modifyvm", :id, "--accelerate3d", "on"]
    end

    # basic provision inline for the system locale	
	#main.vm.provision "Set System Locale", privileged: true, type: "shell", inline: "timedatectl set-timezone Europe/Rome && localectl set-x11-keymap us pc105s intl"

    # basic provision with shell script
    #main.vm.provision :shell, path: ".provision/bootstrap_centos7.sh"
